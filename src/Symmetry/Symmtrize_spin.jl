export Symmtrize_spin
"""

"""
function Symmtrize_spin(TB::TightBindModel{HT}, symops::Vector{SymOp}; heps = 1e-2, reps = 1e-3) where {HT <: Real}
	...
	while true
		...
		for symop in symops
			...
			if !isnothing(I)
				...
				let index = index
					push!(hopmap[j, i], t -> conj(t[index]))
				end
				irredhop[end].t += 1
			end
		end
	...
	return SymTightBindModel{HT}(TB.basis, TB.atom_name, TB.atom_location, TB.orb_name, TB.orb_location,
