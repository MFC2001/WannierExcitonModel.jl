...
function WilsonLoop(TB::AbstractTightBindModel, bandindex::Integer = 1; Nkx = 51, Nky = 51) where {T}
	...
	Threads.@threads for kyi in eachindex(ky)
		...
		uu = sum(i -> cispi(2 * ([1, 0, 0] ⋅ TB.orb_location[i])) * conj(uband[1].vectors[i, bandindex]) * uband[end].vectors[i, bandindex], eachindex(TB.orb_location))
		W = uu * W
	...
	Threads.@threads for kxi in eachindex(kx)
		...
		uu = sum(i -> cispi(2 * ([0, 1, 0] ⋅ TB.orb_location[i])) * conj(uband[1].vectors[i, bandindex]) * uband[end].vectors[i, bandindex], eachindex(TB.orb_location))
		W = uu * W
...
function WilsonLoop(TB::AbstractTightBindModel, bandindex::AbstractVector{<:Integer} = [1, 2]; Nkx = 51, Nky = 51) where {T}
	...
	Threads.@threads for kyi in eachindex(ky)
		...
		uu = [sum(i -> cispi(2 * ([1, 0, 0] ⋅ TB.orb_location[i])) * conj(uband[1].vectors[i, m]) * uband[end].vectors[i, n], eachindex(TB.orb_location)) for m in bandindex, n in bandindex]
		W = uu * W
	...
	Threads.@threads for kxi in eachindex(kx)
		...
		uu = [sum(i -> cispi(2 * ([0, 1, 0] ⋅ TB.orb_location[i])) * conj(uband[1].vectors[i, m]) * uband[end].vectors[i, n], eachindex(TB.orb_location)) for m in bandindex, n in bandindex]
		W = uu * W
