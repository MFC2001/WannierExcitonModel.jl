...
export TightBindModel, numatom, Hamiltonian, spinTightBindModel
...
WannierInterActionBase.numorb(TB::TightBindModel) = length(TB.orb_location)
...
"""
	TightBindModel(;
		...
		orbital::Union{AbstractString, wannier90_centres},
		period = Bool[1, 1, 1],
	)

Create an `TightBindModel` object.
The `AbstractString` is the path to aim file, and check docstrings of `HR`, `AbstractReciprocalHoppings`, `Cell` and `wannier90_centres`.
"""
...
function TightBindModel(rh::AbstractReciprocalHoppings, cell::Cell{<:CartesianCoordinates}, orbital::wannier90_centres)
	return TightBindModel(rh, convert(cell), orbital)
end
function TightBindModel(rh::AbstractReciprocalHoppings, cell::Cell{<:ReducedCoordinates}, orbital::wannier90_centres)
...
"""
function Base.merge(TB::TightBindModel, rh::Union{HR, AbstractReciprocalHoppings})
	numorb(TB) == numorb(rh) || error("Mismatched orbital.")
	all_rh = merge(TB.H, BaseReciprocalHoppings(rh))
	return TightBindModel(
		deepcopy(TB.lattice),
		copy(TB.atom_name),
...
function Base.merge(TB::TightBindModel, arrays::Union{AbstractArray{<:Hopping}, Hopping}...)
	all_rh = merge(TB.H, arrays...)
	return TightBindModel(
		deepcopy(TB.lattice),
		copy(TB.atom_name),
		copy(TB.atom_location),
...
function Base.merge!(TB::TightBindModel, rh::Union{HR, AbstractReciprocalHoppings})
	numorb(TB) == numorb(rh) || error("Mismatched orbital.")
	merge!(TB.H, BaseReciprocalHoppings(rh))
	return TB
end
function Base.merge!(TB::TightBindModel, arrays::Union{AbstractArray{<:Hopping}, Hopping}...)
	merge!(TB.H, arrays...)
